...
def test_load_history_invalid_json(tmp_path):
    history_file = tmp_path / "calculation_history.csv"
    df = pd.DataFrame([
        {'input': '1 + 2', 'result': 3.0, 'timestamp': '2025-06-30T22:18:58.123456', 'steps': 'invalid_json'}
    ])
    df.to_csv(history_file, index=False)
    history = CalculationHistory(str(history_file))
    assert len(history.get_history()) == 1
...
def test_load_history_malformed_csv(tmp_path):
    ...
    with open(history_file, 'w') as f:
        f.write("input,result,timestamp,steps\n1 + 2,invalid,2025-06-30T22:18:58.123456,[]\n")
    with pytest.raises(HistoryError, match=r"Failed to load history:.*"):
        CalculationHistory(str(history_file))
...
#     history.register_observer(mock_observer)
...
#     mock_observer.update.assert_called_once_with("calculation_added", {
#         'input': "1 + 2",
#         'result': 3.0,
#         'timestamp': history.get_history().iloc[0]['timestamp'],
#         'steps': [{'input': "1 + 2", 'operation': "+", 'a': 1.0, 'b': 2.0, 'result': 3.0}]
#     })
...
#     history_file = tmp_path / "calculation_history.csv"
#     history = CalculationHistory(str(history_file))
#     with pytest.raises(HistoryError, match="Invalid history index: 0"):
...
def test_load_history_from_file_malformed_csv(tmp_path):
    history_file = tmp_path / "calculation_history.csv"
    backup_file = tmp_path / "history_backups" / "history_20250630_221858.csv"
    os.makedirs(tmp_path / "history_backups")
    with open(backup_file, 'w') as f:
        f.write("input,result,timestamp,steps\n1 + 2,invalid,2025-06-30T22:18:58.123456,[]\n")
    history = CalculationHistory(str(history_file))
    with pytest.raises(HistoryError, match=r"Failed to load history:.*"):
        history.load_history_from_file("history_20250630_221858.csv")
